// Script para extrair sintomas mais pesquisados (AMOSTRA)
// Executa: npx tsx src/scripts/export-sintomas-sample.ts
...
async function exportarSintomasAmostra() {
  console.log('🔍 Iniciando extração de sintomas mais pesquisados (AMOSTRA)...')
  
  try {
    // 1. Busca apenas as últimas 1000 sessões para teste rápido
    console.log('📊 Buscando últimas 1000 sessões de chat...')
    const chatSessions = await prisma.chatSession.findMany({
      ...
      take: 1000 // Limita a 1000 para teste rápido
    })
    ...
    const filename = `sintomas-pesquisados-AMOSTRA-${timestamp}.csv`
