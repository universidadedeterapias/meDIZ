...
function isSectionTitle(line: string): boolean {
  ...
  for (const keyword of sortedKeywords) {
    ...
    // Verifica se a linha começa exatamente com a palavra-chave
    // E termina com dois pontos OU tem no máximo 3 caracteres adicionais (para "LATERALITY" ou "LATERALITY:")
    const exactMatch = upperLine.startsWith(upperKeyword) && 
                      (upperLine.endsWith(':') || 
                       upperLine === upperKeyword || 
                       upperLine.length <= upperKeyword.length + 3)
    
    // Para evitar falsos positivos, só considera "includes" se a linha for muito próxima do tamanho da palavra-chave
    // E não contém palavras que indicam que é conteúdo, não título
    const includesMatch = upperLine.includes(upperKeyword) && 
                         ...
                         !upperLine.includes('DEPENDS') && // Evita "LATERALITY DEPENDS..."
